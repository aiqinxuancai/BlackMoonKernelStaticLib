...
if ([string]::IsNullOrWhiteSpace($SourceRoot)) { $SourceRoot = Join-Path $repositoryRoot 'krnln' }
if ([string]::IsNullOrWhiteSpace($OutputRoot)) { $OutputRoot = Join-Path $repositoryRoot 'adapter\.adapter-build' }
...
function Read-LegacySourceText {
    ...
    if ($bytes.Length -ge 3 -and $bytes[0] -eq 0xEF -and $bytes[1] -eq 0xBB -and $bytes[2] -eq 0xBF) {
        return $encodingUtf8.GetString($bytes, 3, $bytes.Length - 3)
    }
    return $encoding936.GetString($bytes)
}
function Write-SourceUtf8 {
    param(
    ...
    [IO.File]::WriteAllText($Path, $normalized, $encodingUtf8)
}
...
$stdAfx = Read-LegacySourceText $stdAfxPath
...
Write-SourceUtf8 $stdAfxPath $stdAfx
...
foreach ($headerName in @('lib.h', 'lib2.h')) {
    ...
    $header = Read-LegacySourceText $headerPath
    ...
    $header = $header.Replace('DWORD*            m_pdwSubCodeAdr', 'ULONG_PTR*            m_pdwSubCodeAdr')
    Write-SourceUtf8 $headerPath $header
}
...
foreach ($source in Get-ChildItem -LiteralPath $OutputRoot -Filter '*.cpp') {
    $text = Read-LegacySourceText $source.FullName
    ...
    $text = [regex]::Replace(
        ...
        '$1sizeof(void*)')
    Write-SourceUtf8 $source.FullName $text
}
...
$allGeneratedSourceCode = (Get-ChildItem -LiteralPath $OutputRoot -Filter '*.cpp' | ForEach-Object {
    Remove-CppCommentsAndStrings -Text (Read-LegacySourceText $_.FullName)
}) -join "`n"
foreach ($source in Get-ChildItem -LiteralPath $OutputRoot -Filter '*.cpp') {
    $text = Read-LegacySourceText $source.FullName
...
INT bm_register_opaque_handle(void* value) {
    ...
    for (std::size_t attempt = 0; attempt < 0x3fffffffu; ++attempt) {
        ...
        if (token != 0 && g_bmOpaqueHandles.find(token) == g_bmOpaqueHandles.end()) {
...
'@
Write-SourceUtf8 (Join-Path $OutputRoot 'bm_runtime.cpp') $runtimeSource
...
foreach ($sourceName in $commandSources) {
    $sourceText = Read-LegacySourceText (Join-Path $OutputRoot $sourceName)
...
"@
    Write-SourceUtf8 (Join-Path $OutputRoot $comLifecycleSourceName) $comLifecycleSource
}
...
'@
Write-SourceUtf8 (Join-Path $OutputRoot 'bm_wrapper_support.h') $wrapperSupport
...
foreach ($entry in $legacyFunctions.GetEnumerator() | Sort-Object Name) {
    ...
    Write-SourceUtf8 (Join-Path $wrapperDirectory ($legacyName + '.cpp')) ($wrapperSource -join "`r`n")
